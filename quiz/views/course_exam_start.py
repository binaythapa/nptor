...
def _prepare_track_context(request, exam):
    ...
    track = get_object_or_404(
        ExamTrack.objects.filter(slug=track_slug, is_active=True, organization__isnull=True),
        slug=track_slug,
    )
    ...
    item = next((entry for entry in progress["items"] if entry["exam"].id == exam.id), None)
    if item is None or not item["is_unlocked"]:
        messages.info(request, (item or {}).get("lock_reason") or "This exam is currently locked in the track.")
        return False
    request.session["track_exam_context"] = {"track_slug": track.slug, "exam_id": exam.id}
    return True
...
def course_exam_start(request, exam_id):
    ...
    course_access = _course_access_allows_quiz(request.user, course)
    if not course_access:
        messages.info(request, "You do not have access to this course.")
        return redirect("courses:course_detail", slug=course.slug)

    # Course entitlement (including all-access subscription) is sufficient
    # for a quiz lesson; the exam is not an independently purchasable item.
    if not course_access:
    ...
    attempts = UserExam.objects.filter(user=request.user, exam=lesson.exam, submitted_at__isnull=False).count()
    ...
    request.session["course_exam_context"] = {"course_slug": course.slug, "lesson_id": lesson.id}

    try:
        with transaction.atomic():
            existing = (
                UserExam.objects.select_for_update().filter(
                    user=request.user, exam=lesson.exam, submitted_at__isnull=True
                ).first()
            )
            if existing:
                return redirect("quiz:exam_take", user_exam_id=existing.id)
            ue = UserExam.objects.create(user=request.user, exam=lesson.exam)
            questions = allocate_questions_for_exam(lesson.exam, seed=ue.id)
            if not questions:
            ...
            UserAnswer.objects.bulk_create([UserAnswer(user_exam=ue, question=question) for question in questions])
    except Exception:
        messages.error(request, "This exam is not properly configured. Please contact support.")

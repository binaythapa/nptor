...
def subscription_admin_panel(request):
    ...
    subscriptions = Subscription.objects.select_related(
        "user", "organization", "plan", "granted_by"
    ).prefetch_related("entitlements", "entitlements__exam", "entitlements__track").order_by("-created_at")
    coupons = Coupon.objects.all().order_by("-valid_to")

    return render(request, "quiz/student/subscription/dashboard.html", {
        "users": users, "tracks": tracks, "exams": exams, "plans": plans,
        "exam_subs": exam_subs, "track_subs": track_subs,
        "subscriptions": subscriptions, "coupons": coupons, "now": now,
    })
...
def admin_subscribe_track(request):
    ...
    plan = get_plan_for_track(track, request.POST.get("plan_id"))
    if not plan:
        return JsonResponse({"success": False, "error": "No active subscription plan is attached to this track."}, status=400)
    try:
        subscription, entitlement = SubscriptionService.create_or_reactivate_subscription(
            user=user, resource_type=RESOURCE_TRACK, resource=track, plan=plan,
            granted_by=request.user, notes="Admin manual track subscription",
        )
        AccessService.grant_access(user=user, resource_type=RESOURCE_TRACK, resource=track,
                                   source=ResourceAccess.SOURCE_ADMIN, subscription=subscription,
                                   expires_at=subscription.expires_at)
        return JsonResponse({"success": True, "subscription_id": subscription.id, "entitlement_id": entitlement.id})
    except Exception as exc:
        logger.exception("Failed to subscribe user to track")
...
def admin_revoke_track(request):
    ...
    track = get_object_or_404(ExamTrack, id=request.POST.get("track_id"))
    try:
        entitlement = SubscriptionService.get_user_resource_subscription(user=user, resource_type=RESOURCE_TRACK, resource=track)
        if not entitlement:
            return JsonResponse({"success": False, "error": "Track entitlement not found."}, status=404)
        subscription = entitlement.subscription
        entitlement.is_active = False
        entitlement.save(update_fields=["is_active", "updated_at"])
        access_result = AccessService.revoke_access(user=user, resource_type=RESOURCE_TRACK, resource=track,
                                                    source=ResourceAccess.SOURCE_ADMIN, subscription=subscription)
        ...
        return JsonResponse({"success": True, "access_revoked": access_result["access_revoked"], "access_count": access_result["access_count"]})
    except Exception as exc:
        logger.exception("Failed to revoke track subscription")
        return JsonResponse({"success": False, "error": str(exc)}, status=500)
...
def admin_subscribe_exam(request):
    ...
    plan = get_plan_for_exam(exam, request.POST.get("plan_id"))
    if not plan:
        return JsonResponse({"success": False, "error": "No active subscription plan is available for this exam."}, status=400)
    try:
        subscription, entitlement = SubscriptionService.create_or_reactivate_subscription(
            user=user, resource_type=RESOURCE_EXAM, resource=exam, plan=plan,
            granted_by=request.user, notes="Admin manual exam subscription",
        )
        AccessService.grant_access(user=user, resource_type=RESOURCE_EXAM, resource=exam,
                                   source=ResourceAccess.SOURCE_ADMIN, subscription=subscription,
                                   expires_at=subscription.expires_at)
        return JsonResponse({"success": True, "subscription_id": subscription.id, "entitlement_id": entitlement.id})
    except Exception as exc:
        logger.exception("Failed to subscribe user to exam")
        return JsonResponse({"success": False, "error": str(exc)}, status=500)
...
def admin_revoke_exam(request):
    ...
    exam = get_object_or_404(Exam, id=request.POST.get("exam_id"))
    try:
        entitlement = SubscriptionService.get_user_resource_subscription(user=user, resource_type=RESOURCE_EXAM, resource=exam)
        if not entitlement:
            return JsonResponse({"success": False, "error": "Exam entitlement not found."}, status=404)
        subscription = entitlement.subscription
        entitlement.is_active = False
        entitlement.save(update_fields=["is_active", "updated_at"])
        access_result = AccessService.revoke_access(user=user, resource_type=RESOURCE_EXAM, resource=exam,
                                                    source=ResourceAccess.SOURCE_ADMIN, subscription=subscription)
        if not SubscriptionEntitlement.objects.filter(subscription=subscription, is_active=True).exists():
            subscription.status = Subscription.STATUS_CANCELLED
            subscription.cancelled_at = timezone.now()
        ...
        logger.exception("Failed to revoke exam subscription")
        return JsonResponse({"success": False, "error": str(exc)}, status=500)
...
def toggle_track_status(request):
    try:
        track = ExamTrack.objects.get(id=request.POST.get("track_id"))
...
def create_coupon_ajax(request):
    ...
    if Coupon.objects.filter(code=code).exists():
        return JsonResponse({"success": False, "error": "Coupon already exists"}, status=400)
    try:
        Coupon.objects.create(code=code, percent_off=int(percent_off) if percent_off else None,
                              flat_off=Decimal(flat_off) if flat_off else None,
                              valid_from=timezone.now(), valid_to=timezone.now() + timedelta(days=valid_days), is_active=True)
    except Exception as exc:
    ...
    try:
        ...
        if pricing_type == "free":
            ...
            if not monthly_price:
                return JsonResponse({"success": False, "error": "Monthly price required"}, status=400)
            track.monthly_price = monthly_price
            track.lifetime_price = None
        elif pricing_type == "lifetime":
            if not lifetime_price:
                return JsonResponse({"success": False, "error": "Lifetime price required"}, status=400)
            track.lifetime_price = lifetime_price
            track.monthly_price = None
        else:
            return JsonResponse({"success": False, "error": "Invalid pricing type"}, status=400)
        track.save()
        return JsonResponse({"success": True})
    except ExamTrack.DoesNotExist:
        return JsonResponse({"success": False, "error": "Track not found"}, status=404)
...
def admin_update_exam_expiry(request):
    ...
    if request.POST.get("expires_at"):
        ...
        parsed = parse_datetime(request.POST["expires_at"])
        if parsed is None:
...
def admin_add_exam_days(request):
    try:
        ...
        exam = get_object_or_404(Exam, id=request.POST["item_id"])
        ...
        subscription.cancelled_at = None
        subscription.save(update_fields=["expires_at", "status", "cancelled_at", "updated_at"])
        return JsonResponse({"success": True, "expires_at": subscription.expires_at.isoformat() if subscription.expires_at else None})
    except (TypeError, ValueError):
...
def admin_add_track_days(request):
    try:
        user = get_object_or_404(User, id=request.POST["user_id"])
        track = get_object_or_404(ExamTrack, id=request.POST["item_id"])
        days = int(request.POST["days"])
        if days <= 0:
            raise ValueError
        entitlement = SubscriptionService.get_user_resource_subscription(user=user, resource_type=RESOURCE_TRACK, resource=track)
...
class TrackForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if plans and pricing_type != ExamTrack.PRICING_FREE:
            raise forms.ValidationError("Do not use legacy pricing when subscription plans are selected.")
        return cleaned
...
def admin_add_manual_payment(request):
    try:
        ...
        if exam_id and track_id:
            return JsonResponse({"success": False, "error": "Select either Exam or Track, not both"}, status=400)
        if not exam_id and not track_id:
            return JsonResponse({"success": False, "error": "Exam or Track is required"}, status=400)
        user = get_object_or_404(User, id=user_id)
        exam = get_object_or_404(Exam, id=exam_id) if exam_id else None
        track = get_object_or_404(ExamTrack, id=track_id) if track_id else None
        plan = get_object_or_404(SubscriptionPlan, id=plan_id, is_active=True)
        amount = Decimal(plan.price or 0)
        resource_type = RESOURCE_EXAM if exam else RESOURCE_TRACK
        resource = exam or track
        subscription, entitlement = SubscriptionService.create_or_reactivate_subscription(
            user=user, resource_type=resource_type, resource=resource, plan=plan,
            granted_by=request.user,
            notes="Admin manual exam payment" if exam else "Admin manual track payment",
        )
        ...
        payment = Payment.objects.create(subscription=subscription, amount=amount, currency=plan.currency,
                                         status=Payment.STATUS_SUCCESS, provider=Payment.PROVIDER_MANUAL,
                                         transaction_id=reference_id or "", order_id="", user=user,
                                         organization=None, paid_at=timezone.now(), notes="Admin manual payment")
        AccessService.grant_access(user=user, resource_type=resource_type, resource=resource,
                                   source=ResourceAccess.SOURCE_ADMIN, subscription=subscription,
                                   expires_at=subscription.expires_at)
        return JsonResponse({"success": True, "payment_id": payment.id, "subscription_id": subscription.id, "entitlement_id": entitlement.id})
    except Exception as exc:
        logger.exception("Manual payment failed")
        return JsonResponse({"success": False, "error": str(exc)}, status=500)

from django.contrib.auth.decorators import login_required
from django.http import Http404
from django.shortcuts import get_object_or_404, redirect
from django.views.decorators.http import require_POST

from courses.models import Course, CourseEnrollment
from organizations.models import ResourceAccess
from subscriptions.services import AccessService
from subscriptions.services.plan_service import get_plan_for_course


def is_course_free(course):
    """Return True when the course has no active paid plan."""
    plan = get_plan_for_course(course)
    return plan is None or plan.price <= 0


@login_required
@require_POST
def enroll_free_course(request, slug):
    """Enroll a student in a free public course or an assigned organization course."""
    course = get_object_or_404(Course, slug=slug)

    # Organization-assigned courses do not use student payment/subscription.
    # The assignment itself grants access, so enrollment is only the learning
    # relationship needed by the course UI.
    if AccessService.has_course_access(request.user, course):
        enrollment, _ = CourseEnrollment.objects.get_or_create(
            user=request.user,
            course=course,
            defaults={"is_active": True},
        )

        if not enrollment.is_active:
            enrollment.is_active = True
            enrollment.save(update_fields=["is_active"])

        return redirect("courses:course_learn", slug=course.slug)

    # Public free-course enrollment remains subject to the public catalog gate.
    if not (
        course.approval_status == Course.APPROVAL_APPROVED
        and course.is_published
        and course.is_public
    ):
        raise Http404("Course not found.")

    if not is_course_free(course):
        raise Http404("This course requires paid access.")

    enrollment, _ = CourseEnrollment.objects.get_or_create(
        user=request.user,
        course=course,
        defaults={"is_active": True},
    )

    if not enrollment.is_active:
        enrollment.is_active = True
        enrollment.save(update_fields=["is_active"])

    access, _ = ResourceAccess.objects.get_or_create(
        user=request.user,
        resource_type=ResourceAccess.RESOURCE_COURSE,
        course=course,
        source=ResourceAccess.SOURCE_PUBLIC,
        defaults={
            "is_active": True,
        },
    )

    if not access.is_active or access.revoked_at:
        access.is_active = True
        access.revoked_at = None
        access.save(update_fields=["is_active", "revoked_at", "updated_at"])

    return redirect("courses:course_learn", slug=course.slug)

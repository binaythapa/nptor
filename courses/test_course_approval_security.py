...
class CourseApprovalAuthorizationTests(TestCase):
    ...
    def test_submit_allows_only_supported_states(self, can_edit_mock):
        ...
        for status in ("draft", "changes_required", "rejected"):
            self.assertTrue(can_submit_course_for_review(user, self._course(status=status)))
        self.assertFalse(can_submit_course_for_review(user, self._course(status="pending")))
        self.assertFalse(can_submit_course_for_review(user, self._course(status="approved")))
    ...
    @patch("courses.services.course_approval.can_publish_course", return_value=False)
    def test_publish_service_rejects_non_admin(self, permission_mock):
        user = self._user()

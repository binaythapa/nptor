...
def _create_conversation(cv, purpose, provider, payload, result, metadata=None):
    conversation = AIConversation.objects.create(owner=cv.owner, cv=cv, purpose=purpose, provider=getattr(provider, "name", provider.__class__.__name__), model=getattr(provider, "model", ""), metadata=metadata or {})
...
def _create_suggestions(conversation, result):
    for item in result.get("suggestions", []):
        if not isinstance(item, dict) or not item.get("field_name") or not item.get("section"):
            continue
        AISuggestion.objects.create(conversation=conversation, section=str(item["section"]), field_name=str(item["field_name"]), kind=str(item.get("kind", "improvement")), title=str(item.get("title", "Improve this section")), reason=str(item.get("reason", "")), current_value=item.get("current_value", ""), proposed_value=item.get("proposed_value", ""))
...
def review_cv(cv, provider=None):
    ...
    result = _generate_structured(provider, "Review this CV for ATS readiness, clarity, relevance, and professional impact.\n" f"CV DATA:\n{json.dumps(payload, ensure_ascii=False)}", CV_REVIEW_CONVERSATION_SCHEMA, system_prompt=("You are a professional CV reviewer. Review only the supplied CV data. Never invent employers, dates, qualifications, metrics, skills, or achievements. Return actionable suggestions only when they are supported by the supplied CV. Proposed wording must not introduce unsupported facts."))
...
def analyze_ats(cv, job_description, provider=None):
    ...
    provider = provider or get_provider()
    payload = _review_payload(build_cv_payload(cv))
    result = _generate_structured(provider, "Compare this CV with the supplied job description.\n" f"CV DATA:\n{json.dumps(payload, ensure_ascii=False)}\n\n" f"JOB DESCRIPTION:\n{job_description}", ATS_ANALYSIS_SCHEMA, system_prompt=("You are an ATS and recruitment analyst. Compare only the supplied CV data against the supplied job description. Never invent qualifications, employers, dates, metrics, skills, or experience. Missing keywords must genuinely be absent from the supplied CV. Recommendations must say to add something only if it is truthful."))
    try: score = max(0, min(100, int(result.get("score", 0))))
    except (TypeError, ValueError) as exc: raise AIProviderError("AI provider returned an invalid ATS score.") from exc
...
def tailor_cv(cv, job_description, provider=None):
    ...
    provider = provider or get_provider()
    payload = _review_payload(build_cv_payload(cv))
    result = _generate_structured(provider, "Tailor this CV for the supplied job description without inventing facts.\n" f"CV DATA:\n{json.dumps(payload, ensure_ascii=False)}\n\n" f"JOB DESCRIPTION:\n{job_description}", CV_TAILOR_SCHEMA, system_prompt=("You are a careful CV tailoring specialist. Tailor only the wording supported by the supplied CV. Never invent employers, dates, qualifications, metrics, skills, technologies, or achievements. Prioritize summary and professional title changes that improve relevance without adding unsupported facts."))
...
SAFE_OVERRIDE_SECTIONS = {"summary": {"summary"}, "profile": SAFE_OVERRIDE_FIELDS, "professional_title": {"professional_title"}}
...
def _apply_record_suggestion(suggestion, user):
    ...
    if config is None: raise ValueError("This suggestion cannot be applied automatically")
    model, allowed_fields = config
    if suggestion.field_name not in allowed_fields: raise ValueError("This suggestion field cannot be applied automatically")
    profile = suggestion.conversation.cv.profile
    if profile.user_id != user.id: raise ValueError("Suggestion does not belong to this user")
    current_value = suggestion.current_value
    proposed_value = suggestion.proposed_value
    if not isinstance(current_value, str) or not isinstance(proposed_value, str): raise ValueError("This suggestion has an unsupported value format")
    current_value = current_value.strip()
    proposed_value = proposed_value.strip()
    if not current_value or not proposed_value: raise ValueError("This suggestion must contain both current and proposed text")
    matches = model.objects.filter(profile=profile, **{suggestion.field_name: current_value})
    count = matches.count()
    if count == 0: raise ValueError("The original profile value could not be found. Refresh the AI review and try again.")
    if count > 1: raise ValueError("More than one profile record has this value. This suggestion needs manual review.")
...
def accept_suggestion(suggestion, user):
    ...
    if conversation.owner_id != user.id or not conversation.cv_id: raise ValueError("Suggestion does not belong to this user")
    if suggestion.status != AISuggestion.STATUS_PENDING: return suggestion
...
def reject_suggestion(suggestion, user):
    if suggestion.conversation.owner_id != user.id: raise ValueError("Suggestion does not belong to this user")

...
def _assignment_form_context(organization):
    ...
    exams = Exam.objects.filter(organization=organization).order_by("title")
...
def org_assignment_create(request, slug):
    ...
    if not resource_type:
        messages.error(request, "Please select a resource type.")
        return render(request, "organizations/admin/assignments/create.html", context)
    ...
    if resource_type == ResourceAssignment.RESOURCE_COURSE:
        resource_id = request.POST.get("course_id")
    elif resource_type == ResourceAssignment.RESOURCE_TRACK:
        resource_id = request.POST.get("track_id")
    elif resource_type == ResourceAssignment.RESOURCE_EXAM:
        resource_id = request.POST.get("exam_id")
    else:
    ...
    resource_name = assignment.resource_name or "Resource"
    if result.created:
        messages.success(request, f"{resource_name} was successfully assigned to {student.email}.")
    else:
        messages.success(request, f"{resource_name} assignment for {student.email} was reactivated.")
...
def org_assignment_remove(request, slug, assignment_id):
    ...
    try:
        revoke_assignment(assignment=assignment, actor=request.user, reason=reason)

...
from organizations.permissions import get_active_membership, platform_admin_required
...
def _require_platform_admin(user):
    if not user or not user.is_authenticated or not (user.is_superuser or getattr(getattr(user, "profile", None), "is_platform_admin", False)):
...
def submit_access_request(user, organization, service=OrganizationAccessRequest.SERVICE_ORGANIZATION_ACCESS, requested_role=OrganizationRole.STUDENT, reason=""):
    ...
    request = OrganizationAccessRequest.objects.filter(
    ...
    for admin in User.objects.filter(is_superuser=True).only("id"):
        create_notification(
            ...
            dedupe_key=f"org-access-request:{request.pk}:{admin.pk}",
        )
    return request
...
def approve_access_request(request, reviewer, notes=""):
    ...
    create_notification(
        locked.user, "organization", "Organization access approved",
        f"Your access to {locked.organization.name} has been approved.",
        priority="success", target_url=reverse("organizations_public:workspace", kwargs={"slug": locked.organization.slug}),
        dedupe_key=f"org-access-approved:{locked.pk}",
    )
    return locked
...
def reject_access_request(request, reviewer, notes=""):
    ...
    create_notification(
        locked.user, "organization", "Organization access request declined",
        f"Your request for {locked.organization.name} was not approved.",
        priority="warning", dedupe_key=f"org-access-rejected:{locked.pk}",
    )
    return locked

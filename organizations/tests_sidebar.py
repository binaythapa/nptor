...
class OrganizationSidebarTests(TestCase):
    def setUp(self):
        self.organization = Organization.objects.create(name="Sidebar School", slug="sidebar-school", org_type=Organization.TYPE_SCHOOL, is_active=True)
    ...
    def render_sidebar(self, user):
        ...
        return render_to_string("layouts/student/sidebar.html", {"user": user, "is_from_course": False}, request=request)

    def test_owner_sees_organization_management_link(self):
    ...
    def test_admin_sees_organization_management_link(self):
        html = self.render_sidebar(self.admin)
        self.assertIn("Organization", html)
        self.assertIn("Manage", html)
        self.assertIn("/org/sidebar-school/admin/dashboard/", html)

    def test_staff_sees_organization_workspace_link(self):
        html = self.render_sidebar(self.staff)
        self.assertIn("Organization", html)
        self.assertIn("Workspace", html)
        self.assertIn("/org/sidebar-school/workspace/", html)
        self.assertNotIn("/org/sidebar-school/admin/dashboard/", html)

    def test_student_does_not_see_organization_workspace_link(self):
        html = self.render_sidebar(self.student)
        self.assertNotIn("/org/sidebar-school/workspace/", html)

    def test_owner_membership_is_preferred_when_user_has_multiple_organizations(self):
        other_org = Organization.objects.create(name="Other Sidebar School", slug="other-sidebar-school", org_type=Organization.TYPE_SCHOOL, is_active=True)
        OrganizationMember.objects.create(user=self.owner, organization=other_org, role=OrganizationRole.STUDENT)
        request = self.factory.get("/quiz/dashboard/student/")
